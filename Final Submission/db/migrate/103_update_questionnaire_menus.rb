class UpdateQuestionnaireMenus < ActiveRecord::Migration
  def self.up
    ...
    bookmarkrating_rubrics_action = ControllerAction.find_or_create_by_name('goto_bookmarkrating_rubrics')
    ...
    MenuItem.create(:name => 'manage/questionnaires/bookmark rating rubrics', :label => 'Bookmark rating rubrics', :parent_id => item.id, :seq => 4, :controller_action_id => bookmarkrating_rubrics_action.id)
